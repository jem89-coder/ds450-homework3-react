...
class App extends Component {
  ...
  renderChart() {
    ...
    console.log(data)
    // your code here
    var svg = d3.select(".svg_parent").attr("height", 150).attr("width", 1000);
    if (data.length === 0) {
      return;
    }
    var xScale = d3.scaleLinear().domain([0, data.length - 1]).range([10, 750]);
    var yScale = d3.scaleLinear().domain([0, data.length - 1]).range([85, 85]);
    var fontScale = d3.scaleLinear().domain([0, d3.max(data, d => d[1])]).range([3, 60]);

    svg.selectAll("text").data(data, d => d[0]).join(
      enter =>
        enter.append("text").attr("x", (d, i) => xScale(i)).attr("y", (d, i) => yScale(i))
          .attr("text-anchor", "start").attr("dominant-baseline", "alphabetic").style("font-size", "0px").text(d => d[0])
          .transition().duration(2300).style("font-size", d => fontScale(d[1]) + "px"),
      update =>
        update.transition().duration(2300).attr("x", (d, i) => xScale(i)).attr("y", (d, i) => yScale(i))
